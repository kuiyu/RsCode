...
namespace RsCode.DI
{
    ...
    public static class AttributeBasedServiceCollectionExtensions
    {
        ...
        public static void AddServicesWithAttributeOfType(this IServiceCollection serviceCollection, IEnumerable<Assembly> assembliesToBeScanned)
        {
            if (serviceCollection == null)
            {
                throw new ArgumentNullException(nameof(serviceCollection));
            ...
            List<Type> servicesToBeRegistered = assembliesToBeScanned
                ...
                && !type.FullName.StartsWith("AspNetCoreGeneratedDocument.")
                && !type.FullName.EndsWith(".Startup")
                && !type.FullName.EndsWith(".Program")
                && !type.FullName.EndsWith(".Startup+<>c")
                && !type.FullName.EndsWith(".Program+<>c")
